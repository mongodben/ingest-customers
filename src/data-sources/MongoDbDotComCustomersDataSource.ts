...
/**
  @fileoverview This file contains the data source for pulling mongodb.com/customers
...
export function makeMongoDbDotComCustomersDataSource (
  { connectionUri, dbName }:
  { connectionUri: string, dbName: string }): DataSource {
  return {
    name: 'mongodb-dot-com-customers',
    async fetchPages () {
      ...
      try {
        ...
        const parsedCustomerPages = customerPages.map(
          customerPage => parseCustomerPage({
            input: customerPage,
            sourceName: 'mongodb-dot-com-customers',
            baseUrl: 'https://www.mongodb.com/customers'
          }))
...
function parseCustomerPage ({ input, sourceName, baseUrl }: { input: CustomerPage, sourceName: string, baseUrl: string }): Page {
  assert(input.body ?? input.components, 'input must have `body` or `components`')
...
function makePageTitle (input: CustomerPage): string | undefined {
  ...
  // recurse!
  if (input.components) {
    return findPageTitleInTree(input.components as unknown as DotComCmsComponent)
  }
}
